...
import { ArrowLeft, ArrowRight, RotateCcw, Loader2, GraduationCap, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function FlashcardsPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const [qs, cats] = await Promise.all([fetchQuestions(), fetchCategories()]);
        setQuestions(qs);
        setCategories(cats);
      } finally {
        setLoading(false);
